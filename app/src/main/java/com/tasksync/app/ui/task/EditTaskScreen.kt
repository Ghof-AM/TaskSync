...
fun EditTaskScreen(
    ...
    // Form state — diinisialisasi dari task yang ada
    ...
    // Load task dan member saat screen dibuka
    LaunchedEffect(taskId) {
        viewModel.loadTaskForEdit(taskId)
        viewModel.loadProjectMembers(projectId)
    }

    // Inisialisasi form dari data task yang sudah ada
    LaunchedEffect(taskDetail) {
        if (taskDetail != null && !isInitialized) {
            val task = taskDetail!!
            title = task.title
    ...
    // Setelah members loaded, set selected member dari assignedTo task
    LaunchedEffect(projectMembers, taskDetail) {
        if (projectMembers.isNotEmpty() && taskDetail != null && isInitialized) {
            val assignedId = taskDetail!!.assignedTo
            if (assignedId.isNotBlank()) {
                selectedMember = projectMembers.find { it.userId == assignedId }
            }
        }
    }
        ...
        when (editState) {
            ...
            is UiState.Error -> {
                snackbarHostState.showSnackbar(
                    (editState as UiState.Error).message
                )
                viewModel.resetEditState()
            }
            else -> {}
    ...
    Scaffold(
        ...
        Column(
            ...
            ExposedDropdownMenuBox(
                ...
                OutlinedTextField(
                    value = when {
                        selectedMember == null -> "Tidak di-assign"
